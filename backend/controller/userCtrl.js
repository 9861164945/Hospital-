...
const applyEmployeeController = async (req, res) => {
  try {
    ...
    notifications.push({
      ...
      message: `${newEmployee.firstName} ${newEmployee.lastName} Has Applied For A Job Oppurtunity`,
      data: {
        employee_id: newEmployee._id,
        name: newEmployee.firstName + " " + newEmployee.lastName,
        onClickPath: "/admin/employee",
      },
    });
    await userModel.findByIdAndUpdate(adminUser._id, { notifications });
    res.status(201).send({
    ...
    res.status(500).send({
      success: false,
      error,
      message: "Error WHile Applying For AN EMployee",
    });
  }
};
...
const applyEmployeeController = async (req, res) => {
  try {
    ...
    notifications.push({
      type: "apply-employee-request",
      message: `${newEmployee.firstName} ${newEmployee.lastName} has applied for a job opportunity`,
      data: {
        employee_id: newEmployee._id,
        name: `${newEmployee.firstName} ${newEmployee.lastName}`,
        onClickPath: "/admin/employee",
